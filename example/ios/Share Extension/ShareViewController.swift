...
class ShareViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        // This will be called after the user selects app from sharing app list.
        handleImageAttachment()
    }
    
    func handleImageAttachment(){
        guard let content = self.extensionContext?.inputItems.first as? NSExtensionItem, let contents = content.attachments else { return }
    ...
    private func handleText (content: NSExtensionItem, attachment: NSItemProvider, index: Int) {
        attachment.loadItem(forTypeIdentifier: textContentType, options: nil) { [weak self] data, error in
            guard error == nil, let item = data as? String, let this = self else {
    ...
    private func handleUrl (content: NSExtensionItem, attachment: NSItemProvider, index: Int) {
        attachment.loadItem(forTypeIdentifier: urlContentType, options: nil) { [weak self] data, error in
            guard error == nil, let item = data as? URL, let this = self else {
                self?.dismissWithError()
                return
            }
    ...
    private func handleImages (content: NSExtensionItem, attachment: NSItemProvider, index: Int) {
        attachment.loadItem(forTypeIdentifier: imageContentType, options: nil) { [weak self] data, error in
            guard  error == nil, let url = data as? URL, let this = self else {
                self?.dismissWithError()
                return
            }
    ...
    private func getSharedMediaFile(forVideo: URL)  -> SharingFile? {
        ...
        if FileManager.default.fileExists(atPath: thumbnailPath.path) {
            return SharingFile(value: forVideo.absoluteString,
